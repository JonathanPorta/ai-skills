...
3. Treat “everything” as every project-authored handoff deliverable: editable sources, exports, prototypes, images, icons, tokens, code, fixtures, fonts or audio that may legally ship, specifications, implementation notes, and relevant provenance. Exclude version-control internals, dependency trees, disposable caches, OS metadata, temporary files, and earlier handoff ZIPs. Preserve relative paths from the project root so the ZIP is a root overlay, not a wrapper directory.
...
7. Trust completion only after the helper validates the ZIP and prints its SHA-256. If the helper fails because of an unresolved symlink, reserved metadata collision, missing payload, non-monotonic version, or existing destination, fix the cause; do not bypass the guardrail.
